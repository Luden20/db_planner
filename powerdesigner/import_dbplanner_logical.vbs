'******************************************************************************
'* Importa JSON de db_planner y genera un LDM en PowerDesigner.
'* Materializa intersecciones como entidades hijas para usar la migracion
'* nativa de identificadores de PowerDesigner.
'******************************************************************************
Option Explicit

Const ForReading = 1
Const PdLDM_Model = 1598421368
...
Sub Main()
    ...
    jsonPath = Trim(InputBox("Ruta completa del JSON exportado por db_planner:", "Importar db_planner a LDM"))
    ...
    Set model = CreateModel(PdLDM_Model, "|Diagram=LogicalDiagram")
    model.Name = projectName & " LDM"
    model.Code = BuildCode(model.Name, "DB_PLANNER_LDM")
    ...
    ' Indice para resolver FKs de intersecciones
    Dim intersectionParentMap
    Set intersectionParentMap = CreateObject("Scripting.Dictionary")
    ...
    For Each item In ListValues(entities)
        CreateLDMEntity model, diagram, item, entityIndex, entitySymbolIndex
    Next

    For Each item In ListValues(intersections)
        CreateLDMEntity model, diagram, GetObject(item, Array("entity", "Entity")), entityIndex, entitySymbolIndex
    Next

    For Each item In ListValues(relations)
        CreateLDMRelation model, diagram, item, entityIndex, entitySymbolIndex, intersectionByRelation, intersectionParentMap
    Next

    On Error Resume Next
    diagram.AutoLayoutWithOptions 2
    ...
    Output "LDM generado: " & model.Name
    MsgBox "LDM generado: " & model.Name
End Sub

Sub CreateLDMEntity(model, diagram, entityData, entityIndex, entitySymbolIndex)
    ...
    For Each attr In ListValues(attrs)
        CreateLDMAttribute entity, attr
    Next
...
Sub CreateLDMAttribute(entity, attrData)
    Dim attr, descriptionText, keyType
    ...
    On Error Resume Next
    attr.DataType = CStr(GetValue(attrData, Array("type", "Type"), ""))
    Err.Clear
    attr.Mandatory = Not CBool(GetValue(attrData, Array("optional", "Optional"), False))
    Err.Clear
    On Error GoTo 0

    keyType = LCase(CStr(GetValue(attrData, Array("key_type", "KeyType"), "nil")))
    If keyType = "pk" Then
        Call TryMarkAsPK(attr)
    ElseIf keyType = "fk" Then
        Call TryMarkAsFK(attr)
    End If
...
Sub CreateLDMRelation(model, diagram, relationData, entityIndex, entitySymbolIndex, intersectionByRelation, intersectionParentMap)
    ...
    If relationType = "N:N" Then
        If Not intersectionByRelation.Exists(relationId) Then Exit Sub
        ...
        If intersectionEntity Is Nothing Then Exit Sub

        intersectionId = CStr(GetValue(intersectionEntity, Array("id", "Id"), ""))
        If intersectionId = "" Or (Not entityIndex.Exists(intersectionId)) Then Exit Sub

        ' Guardar padres para referencia
        If Not intersectionParentMap.Exists(intersectionId) Then
            Dim parents
            Set parents = CreateObject("Scripting.Dictionary")
            parents.Add "parent1", entity1Id
            parents.Add "parent2", entity2Id
            Set intersectionParentMap(intersectionId) = parents
        End If

        ' 1:N PADRE -> INTERSECCION (identifying)
        CreateIdentifyingLDMRelation model, diagram, entityIndex(entity1Id), entityIndex(intersectionId), entitySymbolIndex, entity1Id, intersectionId, "REL_" & relationId & "_1"
        CreateIdentifyingLDMRelation model, diagram, entityIndex(entity2Id), entityIndex(intersectionId), entitySymbolIndex, entity2Id, intersectionId, "REL_" & relationId & "_2"
        Exit Sub
    End If
    ...
    Call TrySetRelationEndpoints(relationObject, entityIndex(entity1Id), entityIndex(entity2Id))
    ApplyRelationSemantics relationObject, relationType
    AttachRelationSymbol diagram, relationObject
End Sub

Sub CreateIdentifyingLDMRelation(model, diagram, parentEntity, childEntity, entitySymbolIndex, parentId, childId, relationName)
    Dim relationObject

    Set relationObject = model.Relationships.CreateNew
    relationObject.Name = relationName
    relationObject.Code = relationName
    Call TrySetRelationEndpoints(relationObject, parentEntity, childEntity)
    Call TrySetCardinality(relationObject, "1", "N")
    Call TrySetIdentifying(relationObject, True)
    AttachRelationSymbol diagram, relationObject
End Sub

Sub TrySetRelationEndpoints(rel, obj1, obj2)
    On Error Resume Next
    Set rel.Object1 = obj1
    Set rel.Object2 = obj2
    If Err.Number <> 0 Then
        Err.Clear
        Set rel.Entity1 = obj1
        Set rel.Entity2 = obj2
    End If
    Err.Clear
    On Error GoTo 0
End Sub
...
Sub ApplyRelationSemantics(relationObject, relationType)
    Select Case relationType
        Case "1:1"
            Call TrySetCardinality(relationObject, "1", "1")
            Call TrySetIdentifying(relationObject, False)
        Case "1:N"
            Call TrySetCardinality(relationObject, "1", "N")
            Call TrySetIdentifying(relationObject, False)
        Case "N:1"
            ' Invertir: Object1 es el "N", Object2 es el "1"
            ' Pero como ya asignamos Object1=entity1, Object2=entity2,
            ' las cardinalidades van al reves
            Call TrySetCardinality(relationObject, "N", "1")
            Call TrySetIdentifying(relationObject, False)
        Case "1:NP"
            Call TrySetCardinality(relationObject, "1", "N")
            Call TrySetIdentifying(relationObject, True)
...
Sub AttachRelationSymbol(diagram, relationObject)
